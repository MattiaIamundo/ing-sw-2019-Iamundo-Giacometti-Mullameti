package it.polimi.sw2019.controller.weaponeffect;

import it.polimi.sw2019.controller.Game;
import it.polimi.sw2019.events.weaponeffect_controller_events.NanoTracerChooseEv;
import it.polimi.sw2019.events.weaponeffect_controller_events.NanoTracerSetEv;
import it.polimi.sw2019.exception.InvalidSpaceException;
import it.polimi.sw2019.model.Map;
import it.polimi.sw2019.model.Player;
import it.polimi.sw2019.model.weapon_power.NanoTracerMode;
import it.polimi.sw2019.view.Observable;
import it.polimi.sw2019.view.Observer;
import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;

import java.util.ArrayList;
import java.util.logging.Level;
import java.util.logging.Logger;

public class NanoTracerModeContTest {
    private Logger logger = Logger.getLogger("test.NanoTracerModeCont");
    private ArrayList<Player> players;
    private Map map;
    private NanoTracerMode model = new NanoTracerMode();
    private NanoTracerCont controller = new NanoTracerCont(model);

    @Before
    public void setUp(){
        Game game = new Game();

        try {
            game.createMap("zero");
            game.addPlayers("attacker");
            game.addPlayers("target1");
            game.addPlayers("target2");
            game.addPlayers("noTarget");
            map = game.getGameboard().getMap();
            players = (ArrayList<Player>) game.getPlayers();

            players.get(0).setPosition(map.getSpace(0,1));
            players.get(1).setPosition(map.getSpace(0,2));
            players.get(2).setPosition(map.getSpace(0,2));
            players.get(3).setPosition(map.getSpace(0,1));
        }catch (InvalidSpaceException e){
            logger.log(Level.SEVERE, "out of map boundaries");
        }
    }

    @Test
    public void acquireTargetTest(){
        Catcher catcher = new Catcher();
        ArrayList<String> expectedValid = new ArrayList<>();
        ArrayList<String> expectedNotReachable = new ArrayList<>();

        model.addObserver(catcher);
        controller.useEffect(players.get(0), players, map);
        expectedValid.add("target1");
        expectedValid.add("target2");
        expectedNotReachable.add("noTarget");

        Assert.assertArrayEquals(expectedValid.toArray(), catcher.message.getValid().toArray());
        Assert.assertArrayEquals(expectedNotReachable.toArray(), catcher.message.getNotreachable().toArray());
    }

    private class Catcher implements Observer<NanoTracerChooseEv>{
        NanoTracerChooseEv message;

        @Override
        public void update(NanoTracerChooseEv message) {
            this.message = message;
        }
    }

    @Test
    public void updateTest(){
        Thrower thrower = new Thrower("target1");
        ArrayList<Player> expectedToMark = new ArrayList<>();

        thrower.addObserver(controller);
        controller.useEffect(players.get(0), players, map);
        thrower.throwMessage();
        expectedToMark.add(players.get(1));
        expectedToMark.add(players.get(2));

        Assert.assertArrayEquals(expectedToMark.toArray(), model.getTargets().toArray());
        Assert.assertEquals(players.get(1), model.getTarget());
    }

    private class Thrower extends Observable<NanoTracerSetEv>{
        private String target;

        public Thrower(String target) {
            this.target = target;
        }

        public void throwMessage(){
            notify(new NanoTracerSetEv(target));
        }
    }
}
